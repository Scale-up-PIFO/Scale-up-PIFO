...
int dumbbellSize = 16;
const int LINK_CNT = 2*dumbbellSize+1;
const int FLOW_NUM = 50000;
const char *ACCESS_RATE = "100Gbps";
const char *BACKBONE_RATE = "200Gbps";
const char *DATARATE = "100Gbps";
const char *DELAY =
    "3us";
...
void ScheduleFlowInput()
{
    ...
    if (flow_type == WebSearch) {
        ...
        sinkApp = sinkHelper.Install(nodes.Get(dumbbellSize + flow_input.dst));
    } else {

        sinkAddress = InetSocketAddress(
            interfaces[dumbbellSize + flow_input.dst].GetAddress(0), 30012);
    ...
    if (flow_type == WebSearch) {
        ...
        ns3Socket = Socket::CreateSocket(nodes.Get(flow_input.src), TcpSocketFactory::GetTypeId());
        ns3Socket->SetAttribute("SndBufSize", UintegerValue(1438000000));
    } else {

        ns3Socket = Socket::CreateSocket(nodes.Get(flow_input.src), UdpSocketFactory::GetTypeId());
    }
    ...
    if(flow_type == WebSearch){
      app->Setup(
        ns3Socket, sip, saddr, sinkAddress, PKTSIZE, flow_input.byte_count,
        DataRate(DATARATE), flow_input.dport);
    nodes.Get(flow_input.src)->AddApplication(app);
    }
    else{
      app->Setup(
        ns3Socket, sip, saddr, sinkAddress, PKTSIZE+24, flow_input.byte_count,
        DataRate(DATARATE), flow_input.dport);
    nodes.Get(flow_input.src)->AddApplication(app);
    }
  ...
  {
    if (flow_type == WebSearch) {
      string path = "ScaleUP_PIFO/log/DynamicIntervalRRPIFOScheduler_fct100KB_5MB_load70.txt";
      Output.open(path);
...
int main(int argc, char *argv[])
{
  ...
  string webTrafficFile = "./traffic_gen/ScaleUP_Trace/traffic_ws_load80_dumbbell_dport.txt";
  string hadoopTrafficFile = "./traffic_gen/ScaleUP_Trace/traffic_udp_load80_dport.txt";
  CommandLine cmd;
  ...
  for (int i = 1; i < 8; i++){
    ...
    Ptr<MyApp> app = CreateObject<MyApp>();
    app->Setup(
        ns3UdpSocket, sip, saddr, sinkAddress, PKTSIZE+24, 400000000-i,
        DataRate("100Gbps"), i);
    nodes.Get(i)->AddApplication(app);
    app->SetStartTime(Seconds(2.25)- Simulator::Now());
    app->SetStopTime(Seconds(3.5)- Simulator::Now());
  }

  FlowMonitorHelper fmhelper;
  Ptr<FlowMonitor> monitor = fmhelper.Install(allnodes);
...
std::vector<FlowInfo> small_flows_info;
const double SMALL_FLOW_SIZE_THRESHOLD = 512 * 1024;

for (const auto& flow : completed_websearch_flows_info) {
    if (flow.size <= SMALL_FLOW_SIZE_THRESHOLD) {
        small_flows_info.push_back(flow);
    }
}
uint32_t small_flows_count = small_flows_info.size();

ofstream ss_fct("ScaleUP_PIFO/Results/AllDynamicIntervalRRPIFOScheduler_fct100KB_5MB_load70.txt", std::ios::out);
ofstream ss_95("ScaleUP_PIFO/Results/95DynamicIntervalRRPIFOScheduler_fct100KB_5MB_load70.txt", std::ios::out);

sort(small_flows_info.begin(), small_flows_info.end(), FlowComp_regular);

double totalRegularTimeSmall = 0;
for (const auto& info : small_flows_info) {
    totalRegularTimeSmall += info.regular_time;
}
double p95RegularTimeSmall = 0.0;
double avgRegularTimeSmall = 0.0;
double maxRegularTimeSmall = 0.0;

if (small_flows_count > 0) {
    p95RegularTimeSmall = small_flows_info[static_cast<uint32_t>(0.95 * small_flows_count)].regular_time;
    avgRegularTimeSmall = totalRegularTimeSmall / small_flows_count;
    maxRegularTimeSmall = small_flows_info[small_flows_count - 1].regular_time;
}

ss_95 << "--- Small Flows (<100KB) FCT ---" << endl;
ss_95 << "Normalized FCT P95: " << p95RegularTimeSmall << endl;
ss_95 << "Normalized FCT Avg: " << avgRegularTimeSmall << endl;
ss_95 << "Normalized FCT Max: " << maxRegularTimeSmall << endl;
ss_95 << endl;

sort(small_flows_info.begin(), small_flows_info.end(), FlowComp);

double totalTimeSmall = 0;
for (const auto& info : small_flows_info) {
    totalTimeSmall += info.time;
}
double p95TimeSmall = 0.0;
double avgTimeSmall = 0.0;
double maxTimeSmall = 0.0;

if (small_flows_count > 0) {
    p95TimeSmall = small_flows_info[static_cast<uint32_t>(0.95 * small_flows_count)].time;
    avgTimeSmall = totalTimeSmall / small_flows_count;
    maxTimeSmall = small_flows_info[small_flows_count - 1].time;
}

ss_95 << "Absolute FCT P95: " << p95TimeSmall << endl;
ss_95 << "Absolute FCT Avg: " << avgTimeSmall << endl;
ss_95 << "Absolute FCT Max: " << maxTimeSmall << endl;
ss_95 << endl;

sort(completed_websearch_flows_info.begin(), completed_websearch_flows_info.end(), FlowComp_regular);

ss_95 << "--- All Flows FCT ---" << endl;
